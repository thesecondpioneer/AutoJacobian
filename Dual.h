...
namespace dual {
    ...
    struct Dual {
        ...
        EIGEN_STRONG_INLINE Dual(F x_, const Eigen::DenseBase<der> &y_) {
            x = x_;
            y = y_;
        }
        ...
        Dual(Dual<F, N>&& a) noexcept : x(a.x), y(std::move(a.y)) {}
        ...
        inline Dual<F, N> &operator=(Scalar b) {
            x = b;
            y.setZero();
            return *this;
        }

        Dual<F, N>& operator=(const Dual<F, N>& a) = default;

        Dual<F, N>& operator=(Dual<F, N>&& a) noexcept {
            x = a.x;
            y = std::move(a.y);
            return *this;
        }
    ...
    template <typename F, int N, typename std::enable_if<std::is_arithmetic_v<F>, bool>::type = true>
    inline int fpclassify(const Dual<F, N> &a) {
...
// binary + with a scalar
    ...
    inline Dual<F, N> operator+(const Dual<F, N> &a, Scalar b) {
        return Dual<F, N>(a.x + b, a.y);
    }

    template <typename F, typename Scalar, int N, typename std::enable_if<std::is_arithmetic_v<F>, bool>::type = true,
            typename std::enable_if<std::is_arithmetic_v<Scalar>, bool>::type = true>
    inline Dual<F, N> operator+(Scalar a, const Dual<F, N> &b) {
...
// power function with scalar exponent
    template <typename F, int N, typename std::enable_if<std::is_arithmetic_v<F>, bool>::type = true>
    inline Dual<F, N> pow(const Dual<F, N> &a, F b) {
        if (a.x == F(0) && b >= F(1)) {
            if (b > 1) {
                return Dual<F, N>(F());
            }
...
// power function with scalar base
    template <typename F, int N, typename std::enable_if<std::is_arithmetic_v<F>, bool>::type = true>
    inline Dual<F, N> pow(F a, const Dual<F, N> &b) {
        if (a == F(0) && b.x >= F(1)) {
            if (b.x > 1) {
                return Dual<F, N>(F());
            }
...
// cubic root
    ...
    template <typename F, int N, typename std::enable_if<std::is_arithmetic_v<F>, bool>::type = true>
    inline Dual<F, N> hypot(F a, const Dual<F, N> &b) {
        const F hpab = hypot(a, b.x);
        return Dual<F, N>(hpab, b.x / hpab * b.y);
    }

    template <typename F, int N, typename std::enable_if<std::is_arithmetic_v<F>, bool>::type = true>
    inline Dual<F, N> hypot(const Dual<F, N> &a, F b) {
        return hypot(b, a);
    }
    ...
    template <typename F, int N, typename std::enable_if<std::is_arithmetic_v<F>, bool>::type = true>
    inline Dual<F, N> atan2(F a, const Dual<F, N> &b) {
        return Dual<F, N>(atan2(a, b.y), -(a / (a * a + b.x * b.x)) * b.y);
    }

    template <typename F, int N, typename std::enable_if<std::is_arithmetic_v<F>, bool>::type = true>
    inline Dual<F, N> atan2(const Dual<F, N> &a, F b) {
        return atan2(b, a);
    }
